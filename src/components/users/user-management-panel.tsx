...
export function UserManagementPanel({
  ...
  async function addUser() {
    ...
    if (!res.ok) {
      setError(body?.error ?? "Kullanıcı eklenemedi.");
      return;
    }
    setName("");
    router.refresh();
  ...
  async function changeRole(userId: string, newRole: CompanyUserRole) {
    ...
    setBusyUserId(null);
    if (!res.ok) {
      setError(body?.error ?? "Rol değiştirilemedi.");
      return;
    }
    router.refresh();
  }
  ...
  async function deactivate(userId: string) {
    ...
    setBusyUserId(null);
    if (!res.ok) {
      setError(body?.error ?? "Kullanıcı pasife alınamadı.");
      return;
    }
    router.refresh();
  }
  ...
  async function generatePairing(userId: string) {
    setBusyUserId(userId);
    setError(null);
    const res = await fetch(`/api/users/${userId}/pairing`, { method: "POST" });
    const body = await res.json().catch(() => null);
    setBusyUserId(null);
    if (!res.ok) {
      setError(body?.error ?? "Eşleştirme kodu oluşturulamadı.");
      return;
    }
    setPairingLink({ userId, link: body.deepLink, token: body.token });
  }
              ...
              <div key={u.id} className="rounded-lg border border-border p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {isSelf ? <StatusBadge label="Siz" tone="info" /> : null}
                    <StatusBadge label={u.isActive ? "Aktif" : "Pasif"} tone={u.isActive ? "success" : "danger"} />
                    <StatusBadge
                      label={u.telegramLinked ? "Telegram bağlı" : "Telegram bağlı değil"}
                      tone={u.telegramLinked ? "success" : "warning"}
                    />
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {canManage && !isSelf ? (
                      <Select
                        value={u.role}
                        onValueChange={(v) => changeRole(u.id, v as CompanyUserRole)}
                ...
                {pairingLink?.userId === u.id ? (
                  <div className="mt-2">
                    <AlertBanner
                      ...
                      description={pairingLink.link ?? `Token: ${pairingLink.token} (Telegram botu henüz tanımlı değil)`}
                    />
                  </div>
                ) : null}
              </div>
