...
export default async function BankalarPage() {
  ...
  const [banksWithStats, identity] = await Promise.all([
    bankRepository.listWithStats(ctx),
    ...
    <AppShell userName={identity.userName} companyName={identity.companyName}>
      ...
      {banksWithStats.length === 0 ? (
        <EmptyState
          ...
          action={
            <Button asChild size="lg">
              <Link href="/panel/bankalar/yeni">Banka Ekle</Link>
            </Button>
          }
        />
      ) : (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {banksWithStats.map(({ bank, stats }) => (
            <BankCard
              ...
              status={stats.status}
              openBankHref={`/panel/bankalar/${bank.id}`}
              addPosHref={`/panel/bankalar/${bank.id}/pos/yeni`}
              viewTariffsHref={`/panel/bankalar/${bank.id}?tab=tarifeler`}
            />
          ))}
        </div>
      )}
